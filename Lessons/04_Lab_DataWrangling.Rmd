...
View(NTL.phys.data)
NTL.phys.data.1999tuesdaylake<- filter(NTL.phys.data, year4 %in% c(1990:1999),lakename %in% c("Tuesday Lake"))
View(NTL.phys.data.1999tuesdaylake)
```
...
NTL.phys.data.tuesday.19901999.july<- 
 NTL.phys.data %>%
...
NTL.phys.data.ex4<- NTL.phys.data.tuesday.19901999.july %>%
  filter(depth == 0) %>%
  filter(!is.na(temperature_C)) %>%
  summarise(mean = mean(temperature_C)) #produces just one data point
