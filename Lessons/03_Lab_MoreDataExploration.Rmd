...
```{r}
#creating vectors of high and low temps
hightemps<- c(42, 44, 48, 34, 43, 51, 51, 59, 66, 59) 
lowtemps<- c(20, 27, 27, 15, 26, 25, 33, 49, 48, 27)
```
...
```{r}
#converting vector values to celcius and making new vector
hightemps_c<- hightemps-32
lowtemps_c<- lowtemps-32
```
...
```{r}
#creating data frame
dfweather<- cbind(hightemps, lowtemps, hightemps_c, lowtemps_c)
#naming columns
dfweather_complete<-data.frame("high temperature.F" = hightemps, "low temperature.F"=lowtemps, "high temperature.C"= hightemps_c, "low temperature.C" = lowtemps_c)
#viewing data frame
dfweather_complete
```
...
```{r}
summary(dfweather_complete)
sd(dfweather_complete$high.temperature.F)
summary(dfweather_complete$high.temperature.F)
```
